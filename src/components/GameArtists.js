  ...
  componentWillMount() {
    ...
    axios
      ...
      .then((response) => {
        ...
        while (
          response.data.items[random1].popularity ===
          response.data.items[random2].popularity
        ) {
          var random1 = Math.floor(Math.random() * response.data.items.length);
          var random2 = Math.floor(Math.random() * response.data.items.length);
        }
        ...
        <div className='Results'>
          {this.state.acertou && (
            <div className='column'>
              ...
              <div className='desc'>
                {' '}
                {this.state.artista1} possui uma popularidade de{' '}
                {this.state.popularidade1}%{' '}
              </div>
              <div className='desc'>
                {' '}
                {this.state.artista2} possui uma popularidade de{' '}
                {this.state.popularidade2}%{' '}
              </div>

...
import MusicsCard from './MusicsCard.js';
...
class SelectMusic extends React.Component {
    ...
    render(){
        if(this.state.tracks!==""){
            ...
            return(
                <div>
                    <MusicsCard token={this.props.token} musica={this.state.tracks.items[Math.floor(Math.random() * Object.keys(this.state.tracks.items).length)].track} /> {/*.tracksLink*/}
                </div>
            );
        } else {
            return(

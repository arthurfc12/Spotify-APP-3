...
it('should render the artists names on the buttons', async () => {
	...
	await waitFor(() => {
		...
		expect([ ' The Lumineers', ' John Mayer' ]).toContain(app.find('button').at(0).text());
		expect([ ' The Lumineers', ' John Mayer' ]).toContain(app.find('button').at(1).text());
	});
});
afterEach(cleanup);

it("should render 'which artist is more famous?' game", async () => {
	const data = {
		data: {
			items: [
				{
